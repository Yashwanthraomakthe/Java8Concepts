...
public class Task03 {
	...
	public static void main(String[] args) {
		...
		List<Product> filteredProducts = products.stream().filter(product -> product.getPrice() > 1000)
				.filter(product -> "Electronics".equalsIgnoreCase(product.getCategory())).map(product -> {
